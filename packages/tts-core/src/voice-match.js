...
export function voiceNameHints(trackLanguage, gender) {
...
export function scoreVoiceName(voice, trackLanguage, gender) {
...
export function pickBestVoiceName(voices, trackLanguage, gender) {
    if (voices.length === 0)
        return null;
    let best = null;
    ...
    if (best && bestScore <= 0 && gender === 'MALE') {
        const ranked = [...voices].sort((a, b) => scoreVoiceName(b, trackLanguage, gender) - scoreVoiceName(a, trackLanguage, gender));
        return ranked[0] ?? best;
    }
    return best ?? voices[0] ?? null;
}
//# sourceMappingURL=voice-match.js.map