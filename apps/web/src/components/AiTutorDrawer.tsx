...
在外语学习中，避免死记硬背中文对译，而是去感知动作的“矢量方向”与“状态属性”。建议把这道题加入你的专项弱项复习池，明天艾宾浩斯曲线拐点时系统会为你自动推送巩固练习！`;
  ...
  return (
    <Sheet open={isOpen && !!context} onOpenChange={(open) => !open && onClose()}>
      <SheetContent side="right" className="p-0" showClose>
        {context && (
          <>
            <SheetHeader className="bg-amber-500/10 dark:bg-amber-500/15 pr-12">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  ...
                  <SheetDescription>当前聚焦：{context.skillTag}</SheetDescription>
                </div>
              </div>
            </SheetHeader>
            ...
            <div className="p-3 border-t border-amber-900/10 dark:border-amber-500/15">
              <form
                onSubmit={(e) => {
                  e.preventDefault();
                  handleSendMessage();
